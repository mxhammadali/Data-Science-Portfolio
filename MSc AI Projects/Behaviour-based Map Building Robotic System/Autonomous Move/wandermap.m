...
WHEEL_SPEED = 2;
clc;
disp('To Stop Mapping Close the Figure and Press Ctrl+C');
    ...
    if (clientID>-1)
        ...
        while userInput ~=27
            ...
            if (val=='s')
                close();
                RETURN = csim.simxStopSimulation(clientID,csim.simx_opmode_oneshot);
                disp(RETURN);
                break;
            end
            show(currOccMap);
            ...
            x = moveRobotFromProximity(csim,clientID,leftMotor,rightMotor,WHEEL_SPEED,fLeft,fRight,sLeft,sRight,val);
...
function userInput = moveRobotFromProximity(sim,clientID,...
        leftMotor,rightMotor,speed,fLeft,fRight,sLeft,sRight,val)
        clc;
        disp('Press "m" for moving the robot');
        disp('Press "s" for saving the map');
         userInput=waitforbuttonpress;
         val=get(gcf,'CurrentCharacter');
         disp('Current Key Pressed');
         disp(val); 
         lSpeed = 0;
         rSpeed = 0;
         if (val == 'm')
                ...
                if (rDist < 0.00000001) 
                 ...
                 if (lDist < 0.00000001) 
                    if(detectedPoint1(2)<  0.00000001)
                        lSpeed = 1;
                        rSpeed = -1;
                    else
                        lSpeed = 1;
